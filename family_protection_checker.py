...
# ==================== Step 2: 財務狀況 ====================
    ...
    with col2:
        ...
        st.caption(f"每年總支出約: HK$ {monthly_expense * 12:,}")
...
# ==================== Step 4: 分析缺口 ====================
    ...
    market = {"住院": 1200, "手術": 40000, "癌症": 720000, "危疾": 1000000, "意外醫療": 50000, "意外身故": 1000000}
    annual_expense = data.get("monthly_expense", 0) * 12
    edu_needed = data.get("edu_years", 0) * data.get("edu_cost", 0)
    recommended_life = annual_expense * 10 + edu_needed + data.get("mortgage", 0)
    ...
    compare_df = pd.DataFrame({
        "保障項目": ["醫療 - 住院 (每晚)", "醫療 - 手術 (每次)", "醫療 - 癌症 (每年)", "危疾 (一筆過)", "意外醫療 (每年)", "意外身故/傷殘", "人壽保障", "教育基金"],
        "市場/建議水平": [f"${market['住院']:,}", f"${market['手術']:,}", f"${market['癌症']:,}", f"${market['危疾']:,}", f"${market['意外醫療']:,}", f"${market['意外身故']:,}", f"${recommended_life:,.0f}", f"${edu_needed:,.0f}"],
        "你現有": [f"${data['medical_inpatient']:,}", f"${data['medical_surgery']:,}", f"${data['medical_cancer']:,}", f"${data['critical']:,}", f"${data['accident_medical']:,}", f"${data['accident_death']:,}", f"${data['life']:,}", f"${data['edu_fund']:,}"],
        "差額 (不足)": [f"${inpatient_gap:,}", f"${surgery_gap:,}", f"${cancer_gap:,}", f"${critical_gap:,}", f"${accident_med_gap:,}", f"${accident_death_gap:,}", f"${life_gap:,.0f}", f"${edu_gap:,.0f}"]
    })
    ...
    st.subheader("🔍 主要保障缺口")
    gaps = []
    if inpatient_gap > 0: gaps.append(f"醫療住院不足，每晚差額 ${inpatient_gap:,}")
    if surgery_gap > 0: gaps.append(f"醫療手術不足，每次差額 ${surgery_gap:,}")
    if cancer_gap > 0: gaps.append(f"癌症治療保障不足，每年差額 ${cancer_gap:,}")
    if critical_gap > 0: gaps.append(f"危疾保障不足，差額 ${critical_gap:,}")
    if accident_med_gap > 0: gaps.append(f"意外醫療不足，每年差額 ${accident_med_gap:,}")
    if accident_death_gap > 0: gaps.append(f"意外身故保障不足，差額 ${accident_death_gap:,}")
    if life_gap > 0: gaps.append(f"人壽保障不足，差額 ${life_gap:,.0f}")
    if edu_gap > 0: gaps.append(f"教育基金不足，差額 ${edu_gap:,.0f}")
    
    if gaps:
        for g in gaps: st.error(f"⚠️ {g}")
    else: st.success("✅ 各項保障均達到建議水平，無明顯缺口")
    
    st.subheader("💡 建議行動")
    recs = []
    if data["medical_inpatient"] == 0 and data["medical_surgery"] == 0 and data["medical_cancer"] == 0:
        recs.append("完全沒有醫療保險，建議立即配置自願醫保靈活計劃")
    elif inpatient_gap > 0 or surgery_gap > 0 or cancer_gap > 0:
    ...
    elif accident_med_gap > 0 or accident_death_gap > 0:
    ...
    if edu_gap > 0:
        recs.append("教育基金不足，建議每月儲蓄或購買教育儲蓄保險")
    
    for rec in recs: st.info(f"📌 {rec}")
